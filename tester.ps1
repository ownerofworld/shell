...
if($tater.running)
{
    throw "ChalJa is already running, use Stop-Chalja"
}
...
$tater.status_queue.Add("$(Get-Date -format 's') - Start Ho Gya") > $null
$tater.status_queue.Add("IP Address = $IP") > $null

if($HTTPPort -ne 80)
{
    $tater.status_queue.Add("Port = $HTTPPort") > $null
}

if($NBNS -eq 'Y')
{
    $tater.status_queue.Add("Hostname = $Hostname") > $null

    if($NBNSLimit -eq 'N')
    {
        $tater.status_queue.Add("Limiting Disabled") > $null
    }
...
elseif($Trigger -eq 1)
{
    $tater.status_queue.Add("Def Trigger Enabled") > $null
}
elseif($Trigger -eq 2)
{
...
if($ShowHelp -eq 'Y')
{
    $tater.status_queue.Add("Run stopchalja to stop program early") > $null
